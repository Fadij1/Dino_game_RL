...
pygame.init()
SCREEN_HEIGHT = 600; SCREEN_WIDTH = 1100
...
game_speed = 20; x_pos_bg = 0; points = 0; obstacles = []
...
class Dinosaur:
    ...
    def jump(self):
        ...
        if self.jump_vel < -self.JUMP_VEL: self.dino_jump = False; self.jump_vel = self.JUMP_VEL
    def draw(self, SCREEN): SCREEN.blit(self.image, (self.dino_rect.x, self.dino_rect.y))

class Cloud:
    def __init__(self): self.x = 1100 + random.randint(800, 1000); self.y = random.randint(50, 100); self.image = CLOUD; self.width = self.image.get_width()
    def update(self):
        self.x -= game_speed
        if self.x < -self.width: self.x = 1100 + random.randint(2500, 3000); self.y = random.randint(50, 100)
    def draw(self, SCREEN): SCREEN.blit(self.image, (self.x, self.y))

class Obstacle:
    def __init__(self, image, type):
        self.image = image; self.type = type; self.rect = self.image[self.type].get_rect(); self.rect.x = 1100
    def update(self): self.rect.x -= game_speed; 
    def draw(self, SCREEN): SCREEN.blit(self.image[self.type], self.rect)
...
class Bird(Obstacle):
    ...
    def draw(self, SCREEN):
        ...
        SCREEN.blit(self.image[self.index // 5], self.rect); self.index += 1

def background():
    global x_pos_bg
    image_width = BG.get_width(); SCREEN.blit(BG, (x_pos_bg, 380)); SCREEN.blit(BG, (image_width + x_pos_bg, 380))
...
def test(agent):
    ...
    player = Dinosaur(); cloud = Cloud(); game_speed = 20; x_pos_bg = 0; points = 0; obstacles = []
    ...
    while run:
        ...
        if len(obstacles) > 0:
            ...
            if isinstance(obstacles[0], Bird): obs_type = 1
        else: distance = 1000; obs_y = 325
        ...
        # EXECUTE ACTION
        if action == 1: 
            if not player.dino_jump: player.dino_duck = False; player.dino_run = False; player.dino_jump = True
        elif action == 2:
            if not player.dino_jump: player.dino_duck = True; player.dino_run = False; player.dino_jump = False
        else:
            player.dino_duck = False; player.dino_run = True; player.dino_jump = False

        # RENDER
        SCREEN.fill((255, 255, 255))
        player.draw(SCREEN); player.update(pygame.key.get_pressed())
        ...
        # --- FIX IS HERE: Copy list to remove items safely ---
        for obstacle in list(obstacles): 
            obstacle.draw(SCREEN); obstacle.update()
            
            # Collision
            if player.dino_rect.colliderect(obstacle.rect):
                print(f"GAME OVER. Score: {points}"); pygame.time.delay(1000); test(agent); return
            
            # Remove off-screen obstacles
            if obstacle.rect.x < -obstacle.rect.width:
                obstacles.remove(obstacle)

        background(); cloud.draw(SCREEN); cloud.update()
        points += 1
